...
> **About this manual.**  This is the complete reference for ROAST: every option,
> 40 worked examples, the outputs and how to review them.  The examples are
> written in MATLAB syntax and apply to both implementations - see
> [`../python/README.md`](../python/README.md#option-map) for the Python keyword
> for each option.
>
> Since the repository was reorganised, the MATLAB code lives in `matlab/` and
> the data files in `data/`.  Start MATLAB in the repository root and run
> `addpath('matlab'); setup_roast;` before the first call; everything below then
> works unchanged.
...
After you download the zip file, unzip it, launch your Matlab, make sure you are under the root directory of ROAST (i.e., you can see `example/`, `data/`, `lib/` and `matlab/`), run `addpath('matlab'); setup_roast;` and then enter:
...
(see [Example 3](#example-3)). You can pick any electrode from the 10/20, 10/10, 10/05, BioSemi-256, or EGI HCGSN-256 EEG system (see the file [`capLayout.pdf`](../data/capLayout.pdf) under the root directory of ROAST). The unit of the injected current is in milliampere (mA). Make sure
...
either SPM or NiftyReg worked well. To do this, you can open [an interactive 3D graphical user interface (GUI)](images/manualGui.png) that allows you to 
...
the electrode layouts in the file [`capLayout.pdf`](../data/capLayout.pdf) under the root directory of ROAST. Note the unit of
...
mA) from the extended BioSemi-256 system (see [`capLayout.pdf`](../data/capLayout.pdf) under the root
directory of ROAST).
...
E12 and E7; neck electrodes Nk1 and Nk3 (see [`capLayout.pdf`](../data/capLayout.pdf)); and
...
all the transforms of MRI data (re-orientation into RAS, resampling or zero-padding). See [this figure](images/3spaces.jpg) for details.
...
![manualGui](images/manualGui.png)
...
A lot of info are in the [`capLayout.pdf`](../data/capLayout.pdf) file under the ROAST root directory. There you can find the comprehensive layouts of the [10-05](https://www.sciencedirect.com/science/article/pii/S1053811906009724?via%3Dihub#fig6), [BioSemi](https://www.biosemi.com/pics/cap_256_layout_medium.jpg), and [EGI HCGSN](https://www.egi.com/images/stories/manuals/Second%20Batch%20of%20IFUs%20with%20new%20Notified%20Body%20Jan%202019/GSN_tman_8105171-51_20181231.pdf) systems (with my drawings), and also visually-striking 3D renderings of the New York head with these electrodes placed on. So make sure to check it out.
...
![Three spaces involved in ROAST](images/3spaces.jpg)
